...
export default function GameCanvas() {
  ...
  return <div ref={shellRef} className="game-shell">
    ...
    {settingsOpen && <div className="settings-backdrop" onClick={() => setSettingsOpen(false)}><section className="settings-card" role="dialog" aria-modal="true" aria-labelledby="settings-title" onClick={e => e.stopPropagation()}><button className="close" onClick={() => setSettingsOpen(false)} aria-label="Close settings">×</button><div className="eyebrow">Flight preferences</div><h2 id="settings-title">Settings</h2><p>Adjust motion to keep the flight comfortable.</p><div className="setting-row"><div><strong>Reduced motion</strong><small>Turns off the pre-flap anticipation and wing trail.</small></div><button className={`motion-toggle ${reducedMotion ? "is-on" : ""}`} role="switch" aria-checked={reducedMotion} onClick={() => { const next = !reducedMotion; setReducedMotion(next); localStorage.setItem("flappy-dragon-reduced-motion", String(next)); }}><span /></button></div><button className="settings-done" onClick={() => setSettingsOpen(false)}>DONE</button></section></div>}
...
function drawScene(ctx: CanvasRenderingContext2D, w: number, h: number, state: GameState, sky: HTMLImageElement | undefined, dragons: HTMLImageElement[]) {
  const t = state.time; const flapProgress = Math.max(0, Math.min(1, state.dragon.flap)); const poseIndex = state.mode === "ready" ? 0 : flapProgress > 0.66 ? 1 : flapProgress > 0.2 ? 2 : 0; const poseBlend = state.mode === "ready" ? 0 : poseIndex === 1 ? smoothstep((1 - flapProgress) / 0.34) : poseIndex === 2 ? smoothstep((0.66 - flapProgress) / 0.46) : smoothstep(flapProgress / 0.2); const dragon = dragons?.[poseIndex] || dragons?.[0]; const nextDragon = dragons?.[poseIndex === 1 ? 2 : 0] || dragons?.[0]; ctx.save(); if (state.shake > 0) { ctx.translate(Math.random() * state.shake - state.shake / 2, Math.random() * state.shake - state.shake / 2); state.shake *= .9; }
